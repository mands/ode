...
evalModDef fd mod = do
    ...
    unitsCheck <- St.getSysState St.lUnitsCheck
    St.liftExSys $ evalModDef' modEnv fd unitsState unitsCheck mod'
  where
...
evalModDef' gModEnv fileData unitsState unitsCheck mod@(AppMod fModId modArgs) = do
    ...
    (LitMod modData') <- typeCheck gModEnv fileData unitsState unitsCheck lMod
    ...
    processFArgs funArgs = do
      ...
      where
        ...
        interpretArgs modEnv (argName, mod) = do
            ...
            mod' <- case mod of
                (VarMod _) -> evalModDef' gModEnv fileData unitsState unitsCheck mod
                (AppMod _ _) -> evalModDef' gModEnv fileData unitsState unitsCheck mod
...
evalModDef' gModEnv fileData unitsState unitsCheck mod = do
    ...
    mod' <- validate mod >>= rename >>= typeCheck gModEnv fileData unitsState unitsCheck
    return mod'
