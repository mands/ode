...
def getCallName(funcName, vecSize, mathLib):
    if mathLib == MathLib.Intel:
        if vecSize == 1:
            return funcName
        else:
...
def _getThunk_0_1in_1out(vecMathName, callName, vecSize):
    ...
    cog.outl("define {0} @{1}({0} %ins) nounwind alwaysinline readnone {{".format(llvmType, vecMathName))
    cog.outl("entry:")
    cog.outl("  %ret0 = tail call {0} @{1}({0} %ins) nounwind readnone".format(llvmType, callName))
