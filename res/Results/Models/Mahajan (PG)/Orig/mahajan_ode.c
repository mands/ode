...
int main(void)
{
    ...
    FILE* pFile;
    pFile = fopen ("out_ode.txt","w");
    if (!pFile)
    {
        puts("File not opened successfully.");
        return 1;
    }
    ...
    while(1)
    {
        ...
        fprintf(pFile,"%f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \n",	t,
            Ith(y,1),	Ith(y,2),	Ith(y,3),	Ith(y,4),	Ith(y,19),	Ith(y,27),	Ith(y,25),	Ith(y,28),
            Ith(y,29),	Ith(y,30),	Ith(y,31),	Ith(y,32),	Ith(y,33),	Ith(y,26));
            /* Ca_NSR,	Ca_dyad,	Ca_i,		Ca_submem,	Ca_JSR,		K_i,		Na_i,		I_CaL, */
            /* I_Kr,	I_Ks,		I_K1,		I_NaK,		I_to,		V */
